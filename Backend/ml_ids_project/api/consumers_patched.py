...
def _classify_attack_type(rate: float, is_sm_ips_ports: int, proto: str, state: str, 
    ...
    if proto == "TCP":
        if state in ("SYN", "SA") and rate > 100:
    ...
    def sniff_packets(self):
        ...
        def handle_packet(pkt):
            try:
                ...
                sload = (st["sbytes"] / dur) if dur > 0 else 0.0
                dload = (st["dbytes"] / dur) if dur > 0 else 0.0
                ...
                if total_pkts == 1 or total_pkts % 100 == 0 or (now_ts - last_emit) > 1.0:
                    st["last_emit_ts"] = now_ts
                    flow_features = {
                        'dur': dur, 'spkts': st['spkts'], 'dpkts': st['dpkts'],
                        'sbytes': st['sbytes'], 'dbytes': st['dbytes'], 'rate': rate,
                        'sload': sload, 'dload': dload, 'sinpkt': st.get('last_s_dt') or 0,
                        'dinpkt': st.get('last_d_dt') or 0, 'sjit': st.get('sjit') or 0, 'djit': st.get('djit') or 0,
                        ...
                        'dmean': dmean, 'ct_srv_src': ct_srv_src, 'ct_state_ttl': ct_state_ttl,
